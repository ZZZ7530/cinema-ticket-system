...
- `[提示]` 使用青色。
- 選單編號使用黃色。
- 座位表中 `O` 以綠色顯示，`X` 以紅色顯示。

顏色只用於提升可讀性，所有操作仍以文字、數字選單與 O/X 符號為主，不依賴顏色才能使用。
...
建議使用 Windows Terminal、VS Code Terminal 或 CMD。若 PowerShell / CMD 中文顯示異常，可先執行：
...
- UI Polish：終端機框線、ANSI 顏色、訊息與表格排版美化。
